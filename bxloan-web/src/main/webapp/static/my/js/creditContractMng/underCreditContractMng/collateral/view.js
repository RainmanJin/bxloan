define(function(require, exports, module) {
	...
	var view = Backbone.View.extend({
		...
		events : {
			...
			/** 查看抵质押信息 */
			"click button[role='checkPawn']": "checkPawn",
			/** 删除抵质押 */
			"click button[role='deletePawn']": "deletePawn",
			/** 查询抵质押物品*/
			"click button[id='searchCollateral']": "searchCollateral",
			/** 清空抵质押物品搜索条件 */
			"click button[id='resetCollateral']": "resetCollateral",
			/** 新增抵质押物品 */
			"click button[id='addCollateral']": "addCollateral",
		...
		initProjectPawnInfoTable : function() {
			var oTable = $("#projectPawnInfoTable").dataTable({
				sAjaxSource : $$ctx + "underCreditContractMng/searchProjectPawnInfoList",
				bFilter : false,
				bLengthChange : false,
				aoColumnDefs : [ {
					aTargets : [ 3 ],
				...
				} ,//end
				{
					...
					mRender : function(data, type, full) {
						...
						//console.info($("#underProjectId").val()+"====="+full[10])
						if($('#type').val() != 'check'){
							if($("#underProjectId").val()==full[10])
		...
		checkPawn:function(e){
			var $btnSelf = $(e.currentTarget);
			var id = $btnSelf.data("id");
			$.ajax({
				type : 'post',
				url : $$ctx + 'underCreditContractMng/checkPawn',
				data : {
					'id' : id
				},
				success : function(r) {
		...
		deletePawn:function(e){
			...
			utils.button.confirm(null, function(result) {
				if (result) {
					$.ajax({
						type : 'post',
						url : $$ctx + 'underCreditContractMng/deletePawn',
						data : {
							'id' : id
						},
						success : function(r) {
		...
		initCollateralTable : function() {
			...
			var oTable = $("#table").dataTable({
				sAjaxSource : $$ctx + "underCreditContractMng/findCollateralBySearch",
				bFilter : false,
				bLengthChange : false,
				aoColumnDefs : [ {
					aTargets : [ 0 ],
			...
			var oTable = $("#customerTable_addEditDetailDiv").dataTable({
				sAjaxSource : $$ctx + "underCreditContractMng/findCustomersBySearchForCollateral",
				bFilter : false,
				bLengthChange : false,
				aoColumnDefs : [ {
					aTargets : [ 0 ],
			...
			$('#pawnForm').validate({
				...
				submitHandler : function(form) {
					...
					utils.button.ban("#collateral-submit");//禁用按钮
					$.ajax({
						type : 'post',
						url : $$ctx + 'underCreditContractMng/saveProjectPawnInfo',
						data : {
						...
						success : function(result) {
							...
							if (result.success) {
								utils.alert.suc("<b>" + result.msg + "</b>", function(){
		                        	utils.button.reset("#collateral-submit");//启用按钮
		                    	});
		...
		appGuaDebtInterAmtOnChange : function() {
			...
			if(appGuaDebtInterAmt.match(reg) != null) {
				...
				$('#pawnRatio').val(parseFloat(pawnRatio*100).toFixed(4));
			}
		}
	});
	module.exports = view;
