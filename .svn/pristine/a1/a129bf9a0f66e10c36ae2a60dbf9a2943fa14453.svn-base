...
@Transactional
@Service("interestCalCulateService")
public class InterestCalCulateServiceImpl implements InterestCalCulateService {
	...
	@Override
	public List<InterestVO> loanCounter(InterestCalCulateForm interForm)
			throws Exception {
		init();
		// 日利率
		dll = interForm.getRate().divide(GlobalConstants.DCNT, 50,
		...
		try {
			if ("5".equals(interForm.getRepayment())) {// 等额本金
				...
				BigDecimal total = repayPlanDao.findTotalForCounter();
				if (total == null
						|| total.doubleValue() != interForm.getLoanAmount()
								.doubleValue())
					throw new RuntimeException(
							"<strong>还款计划总额必须与申报金额相等！还款计划总额为：" + total
									+ "，申报金额为：" + interForm.getLoanAmount()
									+ "</strong>");

				//List<RepayPlan> repayPlans = repayPlanDao.findByProjectNo(null);
				List<RepayPlanTemp> repayPlans = repayPlanTempDao.findByProjectNo(null);
